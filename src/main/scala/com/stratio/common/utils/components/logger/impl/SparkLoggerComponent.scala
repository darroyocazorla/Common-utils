...
trait SparkLoggerComponent extends LoggerComponent with SparkLogging {
...
trait SparkLogging {
  ...
  // Make the log field transient so that objects with SparkLogging can
  ...
  protected def initializeLogIfNecessary(isInterpreter: Boolean): Unit = {
    if (!SparkLogging.initialized) {
      SparkLogging.initLock.synchronized {
        if (!SparkLogging.initialized) {
